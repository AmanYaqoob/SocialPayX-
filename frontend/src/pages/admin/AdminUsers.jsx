...
import { Search, Edit, UserCheck, UserX, X, Save, Loader2, Heart } from "lucide-react";
    ...
    const loadPosts = async () => {
      ...
      try {
        ...
        if (res.ok) {
          setPosts(data.posts || []);
          const init = {};
          (data.posts || []).forEach(p => { init[p._id] = p.likes.length; });
          setLikesInput(init);
        }
      } catch (_) {}
  ...
  const handleSetLikes = async (postId) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/admin/posts/${postId}/likes`, {
        ...
        body: JSON.stringify({ likesCount: likesInput[postId] }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
      setPosts(prev => prev.map(p => p._id === postId ? { ...p, likes: { length: data.likesCount } } : p));
      toast({ title: "Likes Updated", description: `Post now has ${data.likesCount} likes` });
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-card border border-border rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto px-6 py-4 flex-1">
          ...
          <SectionTitle>Posts & Likes</SectionTitle>
          {postsLoading ? (
            <p className="text-xs text-muted-foreground mb-3">Loading posts…</p>
          ) : posts.length === 0 ? (
            <p className="text-xs text-muted-foreground mb-3">No posts by this user.</p>
          ) : (
            <div className="flex flex-col gap-2 mb-3">
              {posts.map(post => (
                <div key={post._id} className="flex items-center gap-3 bg-muted/40 border border-border rounded-lg px-3 py-2">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-foreground truncate">{post.content || "(no text)"}</p>
                  ...
                  <input
                    type="number"
                    min="0"
                    value={likesInput[post._id] ?? post.likes.length}
                    onChange={e => setLikesInput(s => ({ ...s, [post._id]: parseInt(e.target.value) || 0 }))}
                    className="w-16 px-2 py-1 bg-input border border-border rounded text-sm text-foreground text-center focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                  <button
                    onClick={() => handleSetLikes(post._id)}
                    disabled={likeSaving[post._id]}
                    className="px-2 py-1 btn-gradient rounded text-xs font-medium text-foreground disabled:opacity-60 flex items-center gap-1"
                  >
                    {likeSaving[post._id] ? <Loader2 className="w-3 h-3 animate-spin" /> : "Set"}
                  </button>
                </div>
              ))}
            </div>
      ...
      <div className="p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => loadUsers(searchQuery)} className="px-4 py-2 bg-primary/20 border border-primary/30 rounded-lg text-primary font-medium hover:bg-primary/30 transition-colors">
              Refresh
            </button>
